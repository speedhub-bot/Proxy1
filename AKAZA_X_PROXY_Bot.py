...
DEV_HANDLE = "@akaza_isnt"
DEV_URL    = "https://t.me/akaza_isnt"
VERSION    = "v6.2 — Phone-friendly .env loader"
# Default workspace: a folder called `akaza_data` next to this script.
...
def extract_proxies_from_text(text: str) -> List[Dict[str, Any]]:
    """
    ...
    # We also keep the original "joined" text so that multi-token formats
    # embedded inside a single line (e.g. "host:port|user|pass") still work.
    lines = re.split(r"\r\n|\r|\n", text)
...
def extract_proxies_from_files(file_path: Path) -> Dict[str, List[Dict[str, Any]]]:
    """
...
def register_handlers(application: "Client"):
    ...
    async def _document(client: Client, message: Message):
        ...
        try:
            per_file = extract_proxies_from_files(local_path)
        except Exception as e:
    ...
    @application.on_message(filters.text & ~filters.command(
        ["start", "help", "stats", "about", "clear", "myinfo", "myproxies",
         ...
         "backup", "health", "auditlog", "purgeaudit"]) & filters.private)
